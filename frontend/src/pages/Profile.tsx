...
import { Play, Lock, User, Mail, BookOpen } from "lucide-react";
    ...
    <>
      ...
      <main className="min-h-screen bg-background pt-20 pb-16 md:pb-0 px-4">
        <div className="max-w-4xl mx-auto py-10 space-y-8">
            ...
            {loadingCourses ? (
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {myCourses.map(course => (
                  <Link key={course.id} to={`/course/${course.id}`} className="glass rounded-2xl overflow-hidden hover:shadow-glow hover:-translate-y-1 transition-all group">
                    <div className="relative h-32 overflow-hidden">
                      <img src={course.image} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                      <div className="absolute inset-0 bg-black/30" />
                      <div className="absolute top-2 right-2">
                        {course.isFree ? (
                          <span className="bg-green-500/90 text-white text-xs font-semibold px-2 py-0.5 rounded-full">Free</span>
                        ) : (
                          <span className="gradient-primary text-primary-foreground text-xs font-semibold px-2 py-0.5 rounded-full">Purchased</span>
                        )}
                      </div>
                    </div>
                    <div className="p-4">
                      <p className="text-xs text-primary font-medium mb-1">{course.category}</p>
                      <h3 className="text-sm font-semibold text-foreground line-clamp-2 mb-2 group-hover:text-primary transition-colors">{course.title}</h3>
          ...
          {!loadingCourses && lockedCourses.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <h2 className="text-xl font-bold text-foreground mb-4">Available to Unlock</h2>
              <div className="grid sm:grid-cols-2 gap-4">
                {lockedCourses.map(course => (
                  <Link key={course.id} to={`/course/${course.id}`} className="glass rounded-2xl overflow-hidden hover:shadow-glow hover:-translate-y-1 transition-all group opacity-70 hover:opacity-100">
                    <div className="relative h-32 overflow-hidden">
